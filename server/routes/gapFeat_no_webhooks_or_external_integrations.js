...
// Gap Feature: No webhooks or external integrations
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-no-webhooks-or-external-integrations',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No webhooks or external integrations'.\nProject: AINutritionDietitianAssistant.\nContext: No webhooks or external integrations\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-webhooks-or-external-integrations', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-webhooks-or-external-integrations', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

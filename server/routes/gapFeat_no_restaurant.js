...
// Gap Feature: No \'/restaurant
// No /restaurant-nutrition-lookup (AI identifies chains from photos; returns menu analysis)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-no-restaurant',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No \'/restaurant'.\nProject: AINutritionDietitianAssistant.\nContext: No /restaurant-nutrition-lookup (AI identifies chains from photos; returns menu analysis)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-restaurant', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-restaurant', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

...
// Gap Feature: No \'/dietary
// No /dietary-goal-forecast (predict weight/health outcomes over 12 weeks)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-no-dietary',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No \'/dietary'.\nProject: AINutritionDietitianAssistant.\nContext: No /dietary-goal-forecast (predict weight/health outcomes over 12 weeks)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-dietary', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-dietary', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

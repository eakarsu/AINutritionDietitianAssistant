...
// Gap Feature: No proper database schema migrations visible
// No proper database schema migrations visible
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-no-proper-database-schema-migrations-visible',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No proper database schema migrations visible'.\nProject: AINutritionDietitianAssistant.\nContext: No proper database schema migrations visible\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-proper-database-schema-migrations-visible', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-proper-database-schema-migrations-visible', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {

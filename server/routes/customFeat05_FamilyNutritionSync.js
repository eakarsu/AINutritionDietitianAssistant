// Custom Feature: Family nutrition sync
// Multi-user household meal planning; AI balances different dietary goals in shared meal plan
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / cf-family-nutrition-sync',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_family_nutrition_sync`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Family nutrition sync'.
Context: Multi-user household meal planning; AI balances different dietary goals in shared meal plan
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-family-nutrition-sync/run error:', err);

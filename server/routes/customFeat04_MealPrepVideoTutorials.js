// Custom Feature: Meal prep video tutorials
// AI generates short video clips of meal prep (sourced from YouTube) matched to user\'s weekly plan
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / cf-meal-prep-video-tutorials',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_meal_prep_video_tutorials`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Meal prep video tutorials'.
Context: AI generates short video clips of meal prep (sourced from YouTube) matched to user\'s weekly plan
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-meal-prep-video-tutorials/run error:', err);

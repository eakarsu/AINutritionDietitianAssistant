// Custom Feature: Nutrient gap identification
// Analyze 7-day diary; flag missing micronutrients (iron, B12, etc.); suggest specific foods to add
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / cf-nutrient-gap-identification',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_nutrient_gap_identification`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Nutrient gap identification'.
Context: Analyze 7-day diary; flag missing micronutrients (iron, B12, etc.); suggest specific foods to add
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-nutrient-gap-identification/run error:', err);

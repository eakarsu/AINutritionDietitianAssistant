...
// Gap Feature: No dedicated routes directory (all logic inline in server/index.js
// No dedicated routes directory (all logic inline in server/index.js — refactor needed)
// Project: AINutritionDietitianAssistant
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-no-dedicated-routes-directory-all-logic-inline-in-',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant for the gap feature: 'No dedicated routes directory (all logic inline in server/index.js'.\nProject: AINutritionDietitianAssistant.\nContext: No dedicated routes directory (all logic inline in server/index.js — refactor needed)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-dedicated-routes-directory-all-logic-inline-in-', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-dedicated-routes-directory-all-logic-inline-in-', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
    res.status(500).json({ error: e.message || 'Internal error' });
  }
});

router.get('/history', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    await ensureTable();
    ...
    const r = await _pool.query(
      ...
      ['no-dedicated-routes-directory-all-logic-inline-in-']
    );

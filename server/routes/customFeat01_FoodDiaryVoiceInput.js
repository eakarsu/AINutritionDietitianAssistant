// Custom Feature: Food diary + voice input
// Async voice recording of meals; AI transcribes and extracts food items auto-populate calorie log
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / cf-food-diary-voice-input',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_food_diary_voice_input`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Food diary + voice input'.
Context: Async voice recording of meals; AI transcribes and extracts food items auto-populate calorie log
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-food-diary-voice-input/run error:', err);

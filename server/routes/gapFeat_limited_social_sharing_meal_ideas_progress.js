...
// Gap Feature: Limited social sharing (meal ideas, progress)
// Limited social sharing (meal ideas, progress)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINutritionDietitianAssistant / gap-limited-social-sharing-meal-ideas-progress',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited social sharing (meal ideas, progress)'.\nProject: AINutritionDietitianAssistant.\nContext: Limited social sharing (meal ideas, progress)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-social-sharing-meal-ideas-progress', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-social-sharing-meal-ideas-progress', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
